...
2. **Set the `project_github.webhook_secret` system parameter** to the
   ...
   Parameters > System Parameters**. GitHub signs every webhook
   payload with this secret (`X-Hub-Signature-256` header) and the
   module verifies the signature; requests are rejected when the
   parameter is missing or still set to the demo default.
